...
HwFacts fill_hw_facts_platform() {
    ...
    if (bsd_common::sysctlbyname_int("kern.smp.cores", cores) && cores > 0) {
        if (bsd_common::sysctlbyname_int("kern.smp.threads_per_core", threads_per_core) &&
            threads_per_core > 0) {
    ...
    // GPU via pciconf
    auto gpus = bsd_common::parse_pciconf_gpu_records("pciconf -lv",
                                                      bsd_common::PciconfFormat::FreeBsdStyle);
    ...
    for (const auto& g : gpus) {
        ...
        bsd_common::apply_vendor_device_hints(c, bsd_common::to_lower(g.vendor),
                                              bsd_common::to_lower(g.device), false);
        scored.push_back(c);
    }
